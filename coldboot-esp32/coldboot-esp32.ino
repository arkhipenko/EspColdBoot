...
void setupOTA() {
  ...
  appVersion = String(APPPREFIX) + String((uint32_t)( ESP.getEfuseMac() & 0xFFFFFFFFL ), HEX) + String("-") + String(APPVER);
...
void checkOTA(String aUrl) {
  ...
  t_httpUpdate_return ret = httpUpdate.update( espClient, aUrl );
  switch (ret) {
